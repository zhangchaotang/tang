...
//  获取商品规格
router.get(baseUrl + '/spec/:id', async ctx => {
  let g_id = 50
...
// 获取商品图片
router.get(baseUrl + '/goodsIng/:id', async ctx => {
  let g_id = 50
  let sql = 'SELECT * FROM particulars_img WHERE g_id = ?'
  let row = await db.query(sql, g_id)
  console.log(row)
  ctx.body = {
    code: 200,
...
// 获取商品属性
router.get(baseUrl + '/goodsAttr/:id', async ctx => {
  let g_id = 50
  let sql = 'SELECT * FROM particulars WHERE g_id = ?'
  let row = await db.query(sql, g_id)
  console.log(row)
  ctx.body = {
    code: 200,
...
// 获取商品评论
router.get(baseUrl + '/goodsComment/:id', async ctx => {
  let g_id = 50
  let sql = `SELECT a.content,b.username,b.userimg,b.petName,c.url FROM
             comment a 
             LEFT JOIN user b ON a.u_id = b.id
             LEFT JOIN comment_img c ON a.id = c.comment_id
             WHERE a.g_id = ? `
  let row = await db.query(sql, g_id)
  ctx.body = {
    code: 200,
